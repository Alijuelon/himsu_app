<x-app-layout>
    ...
    <div class="mb-6 flex items-center p-5 bg-gradient-to-r from-green-500 to-green-400 rounded-xl shadow-sm text-white border border-transparent dark:border-white/5">
        ...
        <div class="ml-4">
            <h4 class="text-sm font-medium text-green-50">
                Total Pemasukan
    ...
    <div class="bg-white dark:bg-navy-700 rounded-xl shadow-sm p-6 border border-transparent dark:border-white/5 transition-colors" x-data="{ modalTambah: false, search: '{{ request('search') }}' }">
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
            ...
            <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto items-center">
                {{-- Filter Bulan & Tahun --}}
                <form action="{{ route('admin.bukukas.pemasukan') }}" method="GET" class="flex gap-2 items-center">
                    @if(request('search'))
                        <input type="hidden" name="search" value="{{ request('search') }}">
                    @endif
                    <select name="bulan" onchange="this.form.submit()" class="py-2.5 px-3 bg-lightBg dark:bg-navy-800 border-none rounded-xl text-sm focus:ring-2 focus:ring-green-500 text-gray-700 dark:text-gray-300 transition-all cursor-pointer">
                    ...
                    <select name="tahun" onchange="this.form.submit()" class="py-2.5 px-3 bg-lightBg dark:bg-navy-800 border-none rounded-xl text-sm focus:ring-2 focus:ring-green-500 text-gray-700 dark:text-gray-300 transition-all cursor-pointer">
                    ...
                    @if(request('bulan') || request('tahun'))
                        <a href="{{ route('admin.bukukas.pemasukan', request('search') ? ['search' => request('search')] : []) }}" class="p-2.5 bg-red-50 dark:bg-red-500/10 text-red-500 rounded-xl hover:bg-red-100 dark:hover:bg-red-500/20 transition" title="Reset Filter">
        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm text-gray-600 dark:text-gray-300">
                                                    ...
                                                    <div x-show="openModalEdit" x-transition.scale.origin.bottom class="relative bg-white dark:bg-navy-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden text-left border border-gray-100 dark:border-white/10">
                                                        <div class="px-6 py-4 border-b border-gray-100 dark:border-white/10 flex items-center justify-between bg-amber-50 dark:bg-amber-500/10">
                                                            <h3 class="text-lg font-bold text-amber-600 dark:text-amber-400">
                                                                <i class="fa-solid fa-pen-to-square mr-2"></i> Edit Pemasukan
                                                            </h3>
                                                        ...
                                                        <form action="{{ route('admin.bukukas.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="p-5 space-y-3">
                                                            @csrf @method('PUT')
                                                            <div>
                                                                <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Tanggal Masuk <span class="text-red-500">*</span></label>
                                                            ...
                                                            <div>
                                                                ...
                                                                <select name="kategori" required class="w-full py-1.5 px-3 bg-lightBg dark:bg-navy-900 border border-transparent dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-amber-500 text-gray-700 dark:text-white transition-all text-sm">
                                                                    <option value="Donasi" {{ $item->kategori == 'Donasi' ? 'selected' : '' }}>Donasi / Sumbangan</option>
                                                                    <option value="Sisa Dana Kegiatan" {{ $item->kategori == 'Sisa Dana Kegiatan' ? 'selected' : '' }}>Sisa Dana Kegiatan</option>
                                                                    ...
                                                                    <div x-show="filePreview || '{{ $item->bukti_nota }}'" class="w-12 h-12 rounded-lg overflow-hidden border border-gray-200 dark:border-white/10 bg-gray-50 flex items-center justify-center shrink-0">
                                                                        ...
                                                                        <template x-if="!filePreview && '{{ $item->bukti_nota }}'">
                                                                            @if(Str::endsWith($item->bukti_nota, ['.pdf', '.PDF']))
                                                                                <i class="fa-solid fa-file-pdf text-red-500 text-xl"></i>
                                                                            @else
                                                                                <img src="{{ asset('storage/bukti_kas/'.$item->bukti_nota) }}" class="w-full h-full object-cover">
                                                                            @endif
                                                                        </template>
                                                                    </div>
                                                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="py-10 text-center text-gray-400">Belum ada catatan pemasukan kas.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <div x-show="modalTambah" style="display: none;" class="fixed inset-0 z-[100] flex items-center justify-center px-4">
                ...
                <div x-show="modalTambah" x-transition.scale.origin.bottom class="relative bg-white dark:bg-navy-800 rounded-2xl shadow-2xl w-full max-w-lg overflow-hidden text-left border border-gray-100 dark:border-white/10">
                    ...
                    <form action="{{ route('admin.bukukas.store') }}" method="POST" enctype="multipart/form-data" class="p-5 space-y-3">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Tanggal Masuk <span class="text-red-500">*</span></label>
                            <input type="date" name="tanggal" value="{{ date('Y-m-d') }}" required class="w-full py-1.5 px-3 bg-lightBg dark:bg-navy-900 border border-transparent dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-700 dark:text-white transition-all text-sm">
                        </div>
                        
                        <div>
                            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Kategori <span class="text-red-500">*</span></label>
                            <select name="kategori" required class="w-full py-1.5 px-3 bg-lightBg dark:bg-navy-900 border border-transparent dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-700 dark:text-white transition-all text-sm">
                        ...
                        <div>
                            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Nominal (Rp) <span class="text-red-500">*</span></label>
                            <input type="number" name="nominal" required min="1" placeholder="Contoh: 150000" class="w-full py-1.5 px-3 bg-lightBg dark:bg-navy-900 border border-transparent dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-700 dark:text-white transition-all text-sm">
                        </div>

                        <div>
                            <label class="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1">Keterangan / Rincian</label>
                            <textarea name="keterangan" rows="1" placeholder="Catatan opsional..." class="w-full py-1.5 px-3 bg-lightBg dark:bg-navy-900 border border-transparent dark:border-white/10 rounded-xl focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-700 dark:text-white text-sm"></textarea>
                        </div>
                        ...
                        <div x-data="{
                            ...
                            <div class="flex items-start gap-3">
                                <div class="flex-1">
                                    <input type="file" name="bukti_nota" accept=".pdf,.jpg,.jpeg,.png" @change="handleFile" class="w-full py-1 px-2 bg-lightBg dark:bg-navy-900 border border-transparent dark:border-white/10 rounded-xl text-xs cursor-pointer file:mr-2 file:py-1 file:px-2 file:rounded-lg file:border-0 file:text-xs file:bg-green-50 file:text-green-700 hover:file:bg-green-100">
                                    <p class="text-[10px] text-gray-400 mt-1">Format: PDF, JPG, PNG. Max: 6MB.</p>
                                </div>
                        ...
                        <div class="pt-2 flex justify-end space-x-2">
                            ...
                            <button type="submit" class="px-4 py-1.5 text-white font-bold rounded-xl shadow-sm text-sm transition-colors bg-green-500 hover:bg-green-600">Simpan Data</button>
                        </div>
                    </form>
                </div>
            </div>
