...
<aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" 
    ...
    @elseif (Auth::user()->role === 'ketua')
        <p class="px-4 text-xs font-bold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Menu Ketua Umum</p>
        ...
        <x-nav-link :href="route('dashboard')" :active="request()->routeIs('dashboard')" class="flex items-center px-4 py-3 {{ request()->routeIs('dashboard') ? 'bg-lightBg dark:bg-white/5 text-brand dark:text-brand font-semibold' : 'text-gray-500 dark:text-gray-400 font-medium hover:text-brand dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5' }} rounded-xl transition-colors relative w-full border-none">
            ...
            @endif
            <i class="fa-solid fa-house w-5 text-center text-lg"></i>
            <span class="mx-3">Dashboard Ketua</span>
        </x-nav-link>

        <!-- Dropdown Monitoring Organisasi -->
        @php $isMonitoringActive = request()->routeIs('ketua.laporan.*', 'ketua.anggota.*'); @endphp
        <div x-data="{ open: {{ $isMonitoringActive ? 'true' : 'false' }} }" class="space-y-1">
            <button @click="open = !open" class="flex items-center justify-between px-4 py-3 w-full rounded-xl transition-colors font-medium border-none {{ $isMonitoringActive ? 'bg-lightBg dark:bg-white/5 text-brand dark:text-brand font-semibold' : 'text-gray-500 dark:text-gray-400 hover:text-brand dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5' }}">
                <div class="flex items-center relative">
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" style="display: none;" class="pl-11 pr-4 py-1 space-y-1 relative">
                ...
                <a href="{{ route('ketua.laporan.index') }}" class="relative flex items-center px-3 py-2 text-sm rounded-lg transition-colors {{ request()->routeIs('ketua.laporan.index') ? 'text-brand dark:text-brand font-semibold bg-gray-50 dark:bg-white/5' : 'text-gray-500 dark:text-gray-400 hover:text-brand dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5' }}">
                    @if(request()->routeIs('ketua.laporan.index')) <div class="absolute -left-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-brand"></div> @endif
                    <i class="fa-solid fa-chart-pie w-5 text-center mr-2"></i>
                    Laporan Keuangan
                </a>
                <a href="{{ route('ketua.laporan.laba-rugi') }}" class="relative flex items-center px-3 py-2 text-sm rounded-lg transition-colors {{ request()->routeIs('ketua.laporan.laba-rugi') ? 'text-brand dark:text-brand font-semibold bg-gray-50 dark:bg-white/5' : 'text-gray-500 dark:text-gray-400 hover:text-brand dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5' }}">
                    @if(request()->routeIs('ketua.laporan.laba-rugi')) <div class="absolute -left-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-brand"></div> @endif
                    <i class="fa-solid fa-scale-balanced w-5 text-center mr-2"></i>
                    Laba Rugi
                </a>
                <a href="{{ route('ketua.anggota.index') }}" class="relative flex items-center px-3 py-2 text-sm rounded-lg transition-colors {{ request()->routeIs('ketua.anggota.*') ? 'text-brand dark:text-brand font-semibold bg-gray-50 dark:bg-white/5' : 'text-gray-500 dark:text-gray-400 hover:text-brand dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5' }}">
                    @if(request()->routeIs('ketua.anggota.*')) <div class="absolute -left-3 top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-brand"></div> @endif
                    <i class="fa-solid fa-users w-5 text-center mr-2"></i>
                    Data Anggota
                </a>
            </div>
        ...
        <!-- Dropdown Keuangan Kas Pribadi (Ketua juga bayar kas) -->
        @php $isKeuanganKetuaActive = request()->routeIs('anggota.bayar.*', 'anggota.riwayat.*', 'anggota.saldo.*'); @endphp
        <div x-data="{ open: {{ $isKeuanganKetuaActive ? 'true' : 'false' }} }" class="space-y-1">
            <button @click="open = !open" class="flex items-center justify-between px-4 py-3 w-full rounded-xl transition-colors font-medium border-none {{ $isKeuanganKetuaActive ? 'bg-lightBg dark:bg-white/5 text-brand dark:text-brand font-semibold' : 'text-gray-500 dark:text-gray-400 hover:text-brand dark:hover:text-white hover:bg-gray-50 dark:hover:bg-white/5' }}">
                <div class="flex items-center relative">
                    @if($isKeuanganKetuaActive)
                        <div class="absolute -left-4 top-1/2 -translate-y-1/2 h-8 w-1 bg-brand rounded-r-full"></div>
                    @endif
                    <i class="fa-solid fa-wallet w-5 text-center text-lg"></i>
                    <span class="mx-3">Kas Pribadi</span>
                </div>
                <i class="fa-solid fa-chevron-down text-xs transition-transform duration-300" :class="open ? 'rotate-180' : ''"></i>
            </button>
            <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 translate-y-0" x-transition:leave-end="opacity-0 -translate-y-2" style="display: none;" class="pl-11 pr-4 py-1 space-y-1 relative">
        ...
        <div class="mt-8 mx-2 p-5 bg-gradient-to-br from-brand to-[#868CFF] rounded-xl text-white text-center shadow-lg shadow-brand/30 dark:shadow-none border border-white/10">
            ...
            <p class="text-xs text-white/80 mb-3">Sebagai anggota, Anda juga wajib membayar kas.</p>
